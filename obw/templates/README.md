...
Templates are deliberately **not** auto-loaded into session context. They are plain files copied via shell (`cp` / `sed` + `obsidian create content=$(cat ...)`) so their contents never enter the token budget.
